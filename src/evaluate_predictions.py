...
if __name__ == '__main__':
    ...
    ford_list = os.listdir(args.prediction_path)
    ford_list.sort()
    print(ford_list)

    main_path = args.prediction_path + ford_list[-1]
    out_dir = os.listdir(main_path)
    out_dir.sort()

    metrics = {}

    for i in out_dir:
        prediction_path = main_path + '/' + i

        if i[-3:] == 'csv' and i != 'history.csv':
            print("\n###################")
            ...
            for mode in ("fine", "coarse"):
                ...
                prediction_fname = \
                os.path.splitext(os.path.basename(prediction_path))[0]
                eval_fname = "evaluation_{}.json".format(prediction_fname)
                eval_path = os.path.join(os.path.dirname(prediction_path),
                                         eval_fname)
